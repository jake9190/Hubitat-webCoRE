 ...
 *
 * Last update October 7, 2021 for Hubitat
*/
...
preferences{
	page((sNAME):'pageMain')
	page((sNAME):'pageRun')
	page((sNAME):'pageClear')
	page((sNAME):'pageClearAll')
	page((sNAME):'pageDumpPiston')
	page((sNAME):'pageDumpPiston1')
}
...
@Field static final String sNAME='name'
...
def pageMain(){
	return dynamicPage((sNAME):'pageMain',title:sBLK,install:true,uninstall:(Integer)state.build!=null){
		if(parent==null || !(Boolean)parent.isInstalled()){
			...
			section(sectionTitleStr('General')){
				label([(sNAME):sNAME,title:'Name',required:true,state:(name ? 'complete':sNULL),defaultValue:(String)parent.generatePistonName(),submitOnChange:true])
			}
...
def pageRun(){
	test()
	return dynamicPage((sNAME):'pageRun',title:sBLK,uninstall:false){
		section('Run'){
...
def pageClear(){
	...
	return dynamicPage((sNAME):'pageClear',title:sBLK,uninstall:false){
		section('Clear'){
...
def pageClearAll(){
	...
	return dynamicPage((sNAME):'pageClearAll',title:sBLK,uninstall:false){
		section('Clear All'){
...
def pageDumpPiston1(){
	...
	rtD=null
	pis=null
	return dynamicPage((sNAME):'pageDumpPiston1',title:sBLK,uninstall:false){
...
//	LinkedHashMap pis=recreatePiston(false,true)
	String message=getMapDescStr(rtD.piston)
	rtD=null
	return dynamicPage((sNAME):'pageDumpPiston',title:sBLK,uninstall:false){
...
Map setup(LinkedHashMap data,chunks){
	...
	for(chunk in chunks)app.updateSetting((String)chunk.key,[type:sTEXT,(sVAL):chunk.value])
	app.updateSetting('bin',[type:sTEXT,(sVAL):(String)state.bin ?: sBLK])
	app.updateSetting('author',[type:sTEXT,(sVAL):(String)state.author ?: sBLK])
...
private Integer msetIds(Boolean shorten,node,Integer maxId=0,Map<String,Integer> existingIds=[:],List<Map> requiringIds=[],Integer level=0){
	...
	String nodeT=node?.t
	if(nodeT in [sIF,sACTION,sCONDITION,sWHILE,sREPEAT,sFOR,sEACH,sSWITCH,sEVERY,sRESTRIC,sGROUP,sDO,sON,sEVENT,sEXIT,sBREAK]){
	...
	if(level==0){
		...
		for(item in requiringIds){
			...
			if(eric() && settings.logging?.toInteger()>2){
				...
				log.warn "adding id $maxId for node ${ty}"
			}
		}
...
private void cleanCode(item){
	...
	if((String)item.t in [sC,sS,sV,sE]){ // operand values that don't need g,a
		...
		if(item.a instanceof String)a=item.remove('a')
	}
	...
	if(item.l!=null && item.l instanceof String)a=item.remove('l')
...
void config(Map data){ // creates a new piston
	...
	if((String)data.bin!=sNULL){
		...
		app.updateSetting('bin',[type:sTEXT,(sVAL):(String)state.bin])
	}
	if((String)data.author!=null){
		state.author=(String)data.author
		app.updateSetting('author',[type:sTEXT,(sVAL):(String)state.author])
	}
...
Map setBin(String bin){
	...
	state.bin=bin
	app.updateSetting('bin',[type:sTEXT,(sVAL):bin])
	String typ='setBin'
	clearMyCache(typ)
	return [:]
}
...
Map setLoggingLevel(String level,Boolean clearC=true){
	...
	app.updateSetting('logging',[type:sENUM,(sVAL):mlogging.toString()])
...
Map test(){
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):'test',(sVAL):now()])
	return [:]
}

Map execute(data,source){
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):'execute',(sVAL): source!=null ? source:now(),jsonData:data],false)
	return [:]
}

Map clickTile(index){
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):'tile',(sVAL):index])
	return (Map)state.state ?: [:]
}

Map clearCache(){
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):'clearc',(sVAL):now()])
	return [:]
}

Map clearLogsQ(){
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):'clearl',(sVAL):now()])
	return [:]
}
			...
			if(queue){
				if(event!=null){
					...
					if(event.device!=null){
						myEvent.device=[id:event.device?.id,(sNAME):event.device?.name,label:event.device?.label]
...
@Field volatile static LinkedHashMap<String,LinkedHashMap> theCacheVFLD=[:] // each piston has a map in here
...
void timeHelper(event,Boolean recovery){
	handleEvents([(sDATE):new Date((Long)event.t),(sDEV):location,(sNAME):sTIME,(sVAL):(Long)event.t,schedule:event,recovery:recovery],!recovery)
}

void executeHandler(event){
	handleEvents([(sDATE):event.date,(sDEV):location,(sNAME):'execute',(sVAL):event.value,jsonData:event.jsonData])
}
...
//measure how Long first state access takes
	...
	rtD.tPause=lZERO
	rtD.stats.timing=[t:startTime,d:eventDelay>lZERO ? eventDelay:lZERO,l:elapseT(startTime)]
	...
	if(clearC||clearL){
		...
		while(success && (Long)getPistonLimits.executionTime+(Long)rtD.timestamp-t >(Long)getPistonLimits.scheduleRemain){
			...
			if(evntName==sASYNCREP){
				...
				evntName=sTIME
				evntVal=t.toString()
				event=[(sDATE):(Date)event.date, (sDEV):location,(sNAME):evntName,(sVAL):t, schedule:sch]
				if(!firstTime) {
			...
			def a=schedules.remove(event.schedule)
			...
			if(evntName==sASYNCREP){
				...
				if(ttyp in [ sHTTPR, sSTOREM, 'lifx', sSENDE, sIFTTM ]) {
...
// any queued events?
	...
	while(doSerialization && semName!=sNULL){
		if(!haveLock) { getTheLock(semName,sHNDLEVT+'2'); haveLock=true }
		List<Map> evtQ=(List<Map>)theQueuesVFLD[semName]
		if(!evtQ){
			...
			Map theEvent
			if(!haveLock) { getTheLock(semName,sHNDLEVT+'3'); haveLock=true }
...
// more to restore here?
		...
		Boolean ended=false
		try{
			Boolean allowed=!rtD.piston.r || rtD.piston.r.size()==0 || evaluateConditions(rtD,(Map)rtD.piston,sR,true)
			...
			if(!ended)tracePoint(rtD,sBREAK,lZERO,0)
		}catch(all){
...
private void processSchedules(Map rtD,Boolean scheduleJob=false){
	...
	for(Integer cid in (List<Integer>)rtD.cancelations.conditions){
		a=schedules.removeAll{ cid in (List)it.cs }
	}

	//cancel on piston state change
	if((Boolean)rtD.pistonStateChanged){
		a=schedules.removeAll{ (Integer)it.ps!=0 }
	}
		...
		if(t1>=0){
			...
			if(t1==0 || t2==0) logs=[]
				...
				if(t1>5 && (Integer)state.toString().size()>75000){
					t1 -= Math.min(50L,Math.round(t1/2.0D)).toInteger()
					logs=logs[0..t1]
			...
			switch((String)statement.t){
				...
				case sEVERY:
					...
					if((Integer)rtD.ffTo!=0 || (ownEvent && allowed && !(Boolean)rtD.restricted)){
						...
						//we only execute the every if i=-1 (for rapid timers with large restrictions i.e. every second, but only on Mondays)
						// NOT VALID: we need to make sure we don't block execution while trying to find the next execution scheduled time, so we give up after too many attempts and schedule a rerun with i=-2 to give us the chance to try again at that later time
...
private Boolean executeTask(Map rtD,List devices,Map statement,Map task,Boolean async){
	...
	String mySt=sNULL
	if((Boolean)rtD.eric){
		mySt='executeTask '+(String)task.c+" async:${async} devices: ${devices.size()}"
		myDetail rtD,mySt,1
	}
	//parse parameters
	List params=[]
...
//		if(eric())log.debug "executeTask ${param}"
		switch((String)param.vt){
			...
			default:
				...
				Boolean match=(tt1!=null &&
						((tt1==(String)v.t)|| (t0 instanceof String && tt1 in [sSTR,sENUM,sTEXT])||
	...
	if(delay!=lZERO){
		...
		if(reschedule || async || delay>(Long)getPistonLimits.taskMaxDelay){
			...
			doPause("executeTask: Waiting for ${delay}ms",delay,rtD,true)
		}
	}
	tracePoint(rtD,myS,elapseT(t),delay)
	...
	if(overBy>lZERO){
		...
		doPause("executeTask: Execution time exceeded by ${overBy}ms,Waiting for ${mdelay}ms",mdelay,rtD)
	}
		...
		if(delta!=lZERO){
			...
			switch(intervalUnit){
				...
				case 'w':
					...
					Long currentDay=dyDay //(Integer)(new Date(time)).day
				...
				case 'y':
					...
					Integer year=dyYear //(Integer)date.year
					Integer month=Math.round((intervalUnit=='n' ? dyMon /*(Integer)date.month*/:omy)+(priorActivity ? interval:((nextSchedule<rightNow)? dONE:dZERO))*(intervalUnit=='n' ? dONE:12.0D)).toInteger()
					if(month>=12){
...
private void scheduleTimeCondition(Map rtD,Map condition){
	...
	Long v1 = (Long)evaluateExpression(rtD,(Map)evaluateOperand(rtD,null,(Map)condition.ro),sDTIME).v + (tv1!=null ? (Long)evaluateExpression(rtD,[t:sDURATION,v: tv1.v,vt:(String)tv1.vt],sLONG).v : lZERO)
	...
	Long v2=trigger ? v1 : ((Integer)comparison.p>1 ? ((Long)evaluateExpression(rtD,(Map)evaluateOperand(rtD,null,(Map)condition.ro2,null,false,true),sDTIME).v + (tv2!=null ? (Long)evaluateExpression(rtD,[t:sDURATION,v:tv2.v,vt:(String)tv2.vt]).v : lZERO)) : (String)condition.lo.v==sTIME ? getMidnightTime():v1 )
	Long n=Math.round(dONE*(Long)now()+2000L)
...
private static Long checkTimeRestrictions(Map rtD,Map operand,Long time,Integer level,Integer interval){
	...
	List<Integer> om=level<=2 && operand.om instanceof List && (Integer)((List)operand.om).size()>0 ? (List)operand.om:null
	List<Integer> oh=level<=3 && operand.oh instanceof List && (Integer)((List)operand.oh).size()>0 ? (List)operand.oh:null
	List<Integer> odw=level<=5 && operand.odw instanceof List && (Integer)((List)operand.odw).size()>0 ? (List)operand.odw:null
	List<Integer> odm=level<=6 && operand.odm instanceof List && (Integer)((List)operand.odm).size()>0 ? (List)operand.odm:null
	List<Integer> owm=level<=6 && odm==null && operand.owm instanceof List && (Integer)((List)operand.owm).size()>0 ? (List)operand.owm:null
	List<Integer> omy=level<=7 && operand.omy instanceof List && (Integer)((List)operand.omy).size()>0 ? (List)operand.omy:null
	...
	if(omy!=null && (omy.indexOf(dyMon+1)<0)){
		...
		Integer month=(tI.find{ it>dyMon+1 } ?: 12+tI[0])- 1
	...
	if(odm!=null){
		if(odm.indexOf(dyDate)<0){
			...
			switch(level){
				case 2: //by second
					result=Math.round(interval*(Math.floor((((tI.find{ it>dyDate } ?: lastDayOfMonth+tI[0])-dyDate)*dsecDay-dyHr*dSECHR-dyMins*dSIXTY)/interval)- 2.0D)*dTHOUS)
					break
				case 3: //by minute
					result=Math.round(interval*(Math.floor((((tI.find{ it>dyDate } ?: lastDayOfMonth+tI[0])-dyDate)*dminDay-dyHr*dSIXTY-dyMins)/interval)-2.0D)*dMSMINT)
					break
			}
			return (result>lZERO)? result:-1L
		}
	...
	if(odw!=null && odw.indexOf(dyDay)<0 ){
		...
		switch(level){
			case 2: //by second
				result=Math.round(interval*(Math.floor((((tI.find{ it>dyDay } ?: 7.0D+tI[0])-dyDay)*dsecDay-dyHr*dSECHR-dyMins*dSIXTY)/interval)-2.0D)*dTHOUS)
				break
			case 3: //by minute
				result=Math.round(interval*(Math.floor((((tI.find{ it>dyDay } ?: 7.0D+tI[0])-dyDay)*dminDay-dyHr*dSIXTY-dyMins)/interval)-2.0D)*dMSMINT)
				break
		}
		return (result>lZERO)? result:-1L
	}
	...
	if(oh!=null && oh.indexOf(dyHr)<0 ){
		...
		switch(level){
			case 2: //by second
				result=Math.round(interval*(Math.floor((((tI.find{ it>dyHr } ?: 24.0D+tI[0])-dyHr)*dSECHR-dyMins*dSIXTY)/interval)-2.0D)*dTHOUS)
				break
			case 3: //by minute
				result=Math.round(interval*(Math.floor((((tI.find{ it>dyHr } ?: 24.0D+tI[0])-dyHr)*dSIXTY-dyMins)/interval)-2.0D)*dMSMINT)
				break
		}
		return result>lZERO ? result:-1L
	}
	...
	//suggest an offset to reach the next minute
		...
		result=Math.round(interval*(Math.floor(((tI.find{ it>dyMins } ?: dSIXTY+tI[0])-dyMins-dONE)*dSIXTY/interval)-2.0D)*dTHOUS)
		return result>lZERO ? result:-1L
	}
	return lZERO
}
...
private Long vcmd_setAlarmSystemStatus(Map rtD,device,List params){
	...
	def options=dev?.ac
	List status=options?.find{ (String)it.key==statusIdOrName || (String)it.value==statusIdOrName }?.collect{ [id:(String)it.key,(sNAME):it.value] }

	if(status && (Integer)status.size()!=0){
		sendLocationEvent((sNAME):'hsmSetArm',(sVAL): status[0].id)
	}else{
...
private Long vcmd_executePiston(Map rtD,device,List params){
	...
	if(!wait){
		sendLocationEvent((sNAME):pistonId,(sVAL):selfId,isStateChange:true,displayed:false,linkText:description,descriptionText:description,data:data)
	}
	return lZERO
}
...
private Long vcmd_setHSLColor(Map rtD,device,List params){
	...
	String mstate=(Integer)params.size()>3 ? (String)params[3]:sNULL
	Long delay=(Integer)params.size()>4 ? (Long)params[4]:lZERO
	if(mstate!=sNULL && (String)getDeviceAttributeValue(rtD,device,sSWITCH)!=mstate){
		return lZERO
	}
	executePhysicalCommand(rtD,device,sSCLR,color,delay)
...
void ahttpRequestHandler(resp,Map callbackData){
	...
	handleEvents([(sDATE):new Date(),(sDEV):location,(sNAME):sASYNCREP,(sVAL):callBackC,contentType:mediaType,responseData:data,jsonData:json,responseCode:responseCode,setRtData:setRtData])
}
...
private evaluateOperand(Map rtD,Map node,Map oper,index=null,Boolean trigger=false,Boolean nextMidnight=false){
	if((Boolean)rtD.eric) myDetail rtD,"evaluateOperand $oper",1
	List<Map> values=[]
	Map operand=oper
	...
	switch((String)operand.t){
		...
		case sV: //virtual devices
			...
			switch(oV){
				...
				case sDATE:
				case sDTIME:
					values=[[i:nodeV,v:[t:oV,v:(Long)cast(rtD,now(),oV,sLONG)]]]
					break
				case sMODE:
				case 'hsmStatus':
				case 'alarmSystemStatus':
					values=[[i:nodeV,v:getDeviceAttribute(rtD,(String)rtD.locationId,oV)]]
					break
				case 'hsmAlert':
				...
				case 'systemStart':
					...
					break
				case 'tile':
					values=[[i:nodeV,v:[t:sSTR,v:(rEN==oV ? evntVal:sNULL)]]]
					break
				case 'ifttt':
		...
		case sX: //variable
			if(ovt==sDEV && operand.x instanceof List){
				...
				values=[[i:nodeI,v:[t:sDEV,v:asum]+(ovt ? [vt:ovt]:[:])]]
			}else{
	...
	if(node==null){ // return a Map instead of a List
		...
		if((Boolean)rtD.eric) myDetail rtD,"evaluateOperand $oper result:$ret",-1
		return ret
	}
	if((Boolean)rtD.eric) myDetail rtD,"evaluateOperand $oper result:$values",-1
	return values
}
...
void doStaysProcess(Map rtD,List<Map>schedules,Boolean isStays,Boolean isStaysUnchg,Map condition,Integer conditionNum,Long delay,Boolean result,String dev) {
	...
	if(isStays && result){
		...
		if(!schedules.find{ (Integer)it.s==conditionNum && (!dev || (String)it.d==dev) }){
	...
	if(canc){
		if((Integer)rtD.logging>2) debug "Cancelling any $msgS",rtD
		cancelStatementSchedules(rtD,conditionNum,dev)
	}
	if(schd){
		if((Integer)rtD.logging>2) debug "Adding a $msgS",rtD
		requestWakeUp(rtD,condition,condition,delay,dev)
	}
	if(!schd && !canc){
		if((Integer)rtD.logging>2) debug "Doing nothing found $msgS",rtD
	}
}
...
//if((Boolean)rtD.eric) myDetail rtD,"$r2res  ${myObj(value?.v?.v)}    ${myObj(rvalue?.v?.v)}  $fn $value   $rvalue    $r2value    $tvalue   $tvalue2",-1
		...
		if(oM){
			...
			if(vVD){
				if(res){
					Boolean a=((List)options.devices.matched).push(vVD)
				}else{
					Boolean a=((List)options.devices.unmatched).push(vVD)
				}
			}
		} else {
...
private List<Map> listPreviousStates(device,String attribute,Long threshold,Boolean excludeLast){
	...
	if((Integer)events.size()!=0){
		...
		Long endTime=now()
		Integer sz=(Integer)events.size()
...
private Boolean comp_is_before				(Map rtD,Map lv,Map rv=null,Map rv2=null,Map tv=null,Map tv2=null){ Long offset1=tv ? (Long)evaluateExpression(rtD,[t:sDURATION,v:tv.v,vt:(String)tv.vt],sLONG).v:lZERO; return cast(rtD,(Long)evaluateExpression(rtD,(Map)lv.v,sDTIME).v+2000L,(String)lv.v.t)<cast(rtD,(Long)evaluateExpression(rtD,(Map)rv.v,sDTIME).v+offset1,(String)lv.v.t)}
private Boolean comp_is_after				(Map rtD,Map lv,Map rv=null,Map rv2=null,Map tv=null,Map tv2=null){ Long offset1=tv ? (Long)evaluateExpression(rtD,[t:sDURATION,v:tv.v,vt:(String)tv.vt],sLONG).v:lZERO; return cast(rtD,(Long)evaluateExpression(rtD,(Map)lv.v,sDTIME).v+2000L,(String)lv.v.t)>=cast(rtD,(Long)evaluateExpression(rtD,(Map)rv.v,sDTIME).v+offset1,(String)lv.v.t)}
private Boolean comp_is_between				(Map rtD,Map lv,Map rv=null,Map rv2=null,Map tv=null,Map tv2=null){ Long offset1=tv ? (Long)evaluateExpression(rtD,[t:sDURATION,v:tv.v,vt:(String)tv.vt],sLONG).v:lZERO; Long offset2=tv2 ? (Long)evaluateExpression(rtD,[t:sDURATION,v:tv2.v,vt:(String)tv2.vt],sLONG).v:lZERO; Long v=(Long)cast(rtD,(Long)evaluateExpression(rtD,(Map)lv.v,sDTIME).v+2000L,(String)lv.v.t); Long v1=(Long)cast(rtD,(Long)evaluateExpression(rtD,(Map)rv.v,sDTIME).v+offset1,(String)lv.v.t); Long v2=(Long)cast(rtD,(Long)evaluateExpression(rtD,(Map)rv2.v,sDTIME).v+offset2,(String)lv.v.t); return v1<v2 ? v>=v1 && v<v2 : v<v2 || v>=v1}
...
private void updateDeviceList(Map rtD,List deviceIdList){
	app.updateSetting('dev',[type: 'capability',(sVAL): deviceIdList.unique()])// settings update do not happen till next execution
}
...
private void subscribeAll(Map rtD,Boolean doit=true){
	...
	try{
				...
				case sP: //physical device
					for(String mdeviceId in expandDeviceList(rtD,(List)operand.d,true)){
						...
						if(ct==sTRIG || comparisonType==sTRIG){
							...
							if(doit && (Integer)rtD.logging>2)debug msgVal+' subscription',rtD
		...
		if(doit){
			...
			Map event=[(sDATE):new Date(),(sDEV):location,name:sTIME,(sVAL):now(),schedule:[t:lZERO,s:0,i:-9]]
...
private Map evaluateExpression(Map rtD,Map express,String dataType=sNULL){
	...
	if(!express)return [t:sERROR,v:'Null expression']
	...
	switch(exprType){
		...
		case sINT64:
		case sDATE:
			result=[t:exprType,v:cast(rtD,exprV,exprType,dataType)]
			break
		case sBOOL:
		case sBOOLN:
			if(exprV instanceof Boolean){
				result=[t:sBOOLN,v:(Boolean)exprV]
				break
			}
			Boolean t1=cast(rtD,exprV,sBOOLN,dataType)
			result=[t:sBOOLN,v:t1]
			break
		case sSTR:
		case sENUM:
		case sERROR:
		...
		case sTEXT:
			if(exprV instanceof String){
				result=[t:sSTR,v:(String)exprV]
				break
			}
			result=[t:sSTR,v:(String)cast(rtD,exprV,sSTR,dataType)]
			break
		case sNUMBER:
		case sFLOAT:
		case sDBL:
			if(exprV instanceof Double){
				result=[t:sDCML,v:(Double)exprV]
				break
			}
			result=[t:sDCML,v:(Double)cast(rtD,exprV,sDCML,dataType)]
			break
		case sDURATION:
			String t0=(String)expression.vt
			if(t0==null && exprV instanceof Long){ result=[t:sLONG,v:(Long)exprV ] }
			else result=[t:sLONG,v:(Long)cast(rtD,exprV,t0!=sNULL ? t0:sLONG)]
			break
		case sVARIABLE:
		...
		case sDEV:
			...
			break
		case sOPERAND:
			result=[t:sSTR,v:(String)cast(rtD,exprV,sSTR)]
			break
		case sFUNC:
							...
							switch(sz){
								...
								default:
									for(v in (List)param.v){
										a=params.push([t:(String)param.t, a:(String)param.a, v:[v]])
									}
							}
				...
				if((String)item.t==sOPER){
					...
					if(operand<0){
						switch(ito){
							...
							case sDNEG:
							case sQM:
								a=items.push([t:sINT,v:0,o:ito])
								break
							case sCOLON:
								if(lastOperand>=0){
									//groovy-style support for(object ?: value)
									a=items.push(items[lastOperand]+[o:ito])
								}else{
									a=items.push([t:sINT,v:0,o:ito])
								}
								break
							case sMULP:
							case sDIV:
								a=items.push([t:sINT,v:1,o:ito])
								break
							case sLAND:
							case sLNAND:
								a=items.push([t:sBOOLN,v:true,o:ito])
								break
							case sLOR:
							case sLNOR:
							case sLXOR:
							case sLNXOR:
								a=items.push([t:sBOOLN,v:false,o:ito])
								break
						}
					}else{
						items[operand].o=ito
...
private Map func_replace(Map rtD,List<Map> params){
	...
	String value=(String)evaluateExpression(rtD,params[0],sSTR).v
	Integer cnt=Math.floor(((Integer)params.size()-1)/2).toInteger()
...
private Map func_rangevalue(Map rtD,List<Map> params){
	...
	Map value=params[1]
	Integer cnt=Math.floor(((Integer)params.size()-2)/2).toInteger()
	for(Integer i=0; i<cnt; i++){
...
private Map func_title(Map rtD,List<Map> params){
	...
	return [t:sSTR,v:((List)result.tokenize(sSPC))*.toLowerCase()*.capitalize().join(sSPC)]
}
...
private Map func_variance(Map rtD,List<Map> params){
	...
	sum=dZERO
	Integer sz=(Integer)values.size()
	for(Integer i=0; i<sz; i++){
...
private Map func_stdev(Map rtD,List<Map> params){
	...
	return [t:sDCML,v:Math.sqrt(result.v.toDouble())]
}
	...
	if((String)param.t==sDEV && (String)param.a && (Integer)((List)param.v).size()){
		...
		if(device!=null && !isDeviceLocation(device)){
			...
			if((Integer)states.size()>1){
				...
				Integer sz=(Integer)states.size()
...
private Map func_weekdayname(Map rtD,List<Map> params){
	...
	Integer index=((value>=lMSDAY)? (Integer)utcToLocalDate(value).day:value.toInteger()) % 7
...
private Map func_monthname(Map rtD,List<Map> params){
	...
	Long value=(Long)evaluateExpression(rtD,params[0],sLONG).v
	Integer index=((value>=lMSDAY)? (Integer)utcToLocalDate(value).month: (value-1L).toInteger())%12+1
...
@Field volatile static Map theHashMapVFLD=[:]

private String hashId(id,Boolean updateCache=true){
	String result
	String myId=id.toString()
	result=(String)theHashMapVFLD[myId]
	if(result==sNULL){
		result=sCOLON+md5('core.'+myId)+sCOLON
		if(updateCache){
			getCacheLock(sD)
			theHashMapVFLD[myId]=result
...
private cast(Map rtD,ival,String dataT,String isrcDT=sNULL){
	...
	switch(dataType){
		...
		case sTIME:
			...
			Date t1=new Date(d)
			Long t2=Math.round(((Integer)t1.hours*dSECHR+(Integer)t1.minutes*dSIXTY+(Integer)t1.seconds)*dTHOUS)
			return t2
		case sDATE:
			...
			Date t1=new Date(d)
			Long t2=Math.round((Math.floor(d/dTHOUS)*dTHOUS)-(((Integer)t1.hours*dSECHR+(Integer)t1.minutes*dSIXTY+(Integer)t1.seconds)*dTHOUS)) // take ms off and first guess at midnight (could be earlier/later depending if DST change day
			return t2
		case sDTIME:
			if(srcDataType==sTIME){
				Long tt1=value.toLong()
...
private Map getRandomColor(){
	Integer random=Math.round(Math.random()*((Integer)getColors().size()-1)*dONE).toInteger()
