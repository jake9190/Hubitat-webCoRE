 ...
 *
 * Last update December 16, 2021 for Hubitat
*/
...
Map activity(lastLogTimestamp){
	...
	Integer index=(llt!=lZ && lsz>0)? logs.findIndexOf{ it?.t==llt }:0
	index=index>0 ? index:(llt!=lZ ? 0:lsz)
	Map rVal=[
		(sNM): (String)t0.name,
		state: (Map)t0.state,
...
Map setup(LinkedHashMap data,Map<String,String>chunks){
	...
	for(chunk in ((Map<String,Object>)settings).findAll{ ((String)it.key).startsWith('chunk:') && !chunks[(String)it.key] }){
...
private void cleanCode(item){
	...
	if(ty in ListC3 || (ty==sC && !((String)item.vt in LT1)) ) // operand values that don't need c
		a=item.remove(sC)
	if(ty==null && item.size()==4 && item.d instanceof List && !item.d && (String)item.g=='avg' && item.f==sL && item.vt){
		a=item.remove(sD); a=item.remove(sG)
	}
	if(ty in ListC1){ // operand values that don't need g,a
		if((String)item.g=='avg')a=item.remove(sG)
...
private LinkedHashMap<String,Object> getRunTimeData(LinkedHashMap<String,Object> rtD=null,Map retSt=null,Boolean fetchWrappers=false,Boolean shorten=false){
	...
	LinkedHashMap<String,Object> m1=[semaphore:lZ,semaphoreName:sNULL,semaphoreDelay:lZ] as LinkedHashMap<String,Object>
	if(retSt!=null){
...
void handleEvents(evt,Boolean queue=true,Boolean callMySelf=false){
	...
	Long eventDelay=Math.round(dONE*startTime-((Date)event.date).getTime())
...
//measure how Long first state access takes
	...
	if(clearC||clearL){
		...
		while(success && (Long)getPistonLimits.executionTime+(Long)rtD.timestamp-(Long)now()>(Long)getPistonLimits.scheduleRemain){
			...
			schedules=sgetSchedules(sHNDLEVT+sONE,myPep)
			a=schedules.remove(event.schedule)

			updateSchCache(rtD, schedules, sHNDLEVT+sONE, sX, myPep)
			tt0=null
...
// more to restore here?
		...
		try{
					...
					if(!(Boolean)rtD.restricted){
						...
						if(data!=null && (String)data.d && (String)data.c){
							...
							if(device!=null){
								...
								executePhysicalCommand(rtD,device,(String)data.c,data.p,lZ,sNULL,true)
							}
						}
					}
				}else{
			...
			if(!ended){ ended=true; tracePoint(rtD,sBREAK,lZ,0) }
		}catch(all){
...
private void processSchedules(Map rtD,Boolean scheduleJob=false){
	...
	updateSchCache(rtD, schedules, str+sONE, sT, myPep)

	if(scheduleJob){
		Long nextT=lZ
		Integer ssz=schedules.size()
		if(ssz>0){
			Map tnext=schedules.sort{ Map it -> (Long)it.t }[0]
			nextT=(Long)tnext.t
...
private Boolean executeStatement(Map rtD,Map statement,Boolean asynch=false){
	...
	String mySt=sNULL
	if((Boolean)rtD.eric){
		mySt=str+ffwding(rtD)+stateType
		myDetail rtD,mySt,1
	}
			...
			switch(stateType){
				...
				case sFOR:
					...
					if( (startValue<=endValue && stepValue>dZERO) || (startValue>=endValue && stepValue<dZERO) || (Integer)rtD.ffTo!=0){
						...
						if(isEach && (Integer)rtD.ffTo in [0,-9])setSystemVariableValue(rtD,sDLLRDEVICE,index<dsiz ? [devices[index.toInteger()]]:[])
				...
				case sSWITCH:
					...
					Boolean found=false
					Boolean implicitBreaks= (String)statement.ctp==sI // case traversal policy, i - autobreak (def), e - fall thru
				...
				case sDO:
					...
					break
				case sBREAK:
					if((Integer)rtD.ffTo==0) rtD.break=true
					value=false
					break
				case sEXIT:
					if((Integer)rtD.ffTo==0){
	...
	if((Integer)rtD.ffTo==0){
		...
		if(stateType==sEVERY){
			...
			if(t0==null){
				List<Map> schedules=sgetSchedules(str+sONE,myPep)
...
private Boolean executeAction(Map rtD,Map statement,Boolean async){
	...
	//if override
	if((Integer)rtD.ffTo==0 && (String)statement.tsp!=sA) // Task scheduling policy - a - allow multiple schedules, ""-override existing (def)
	...
	for(Map task in (List<Map>)statement.k){
		...
		if(!result && (Integer)rtD.ffTo==0) break
	}
	...
	for(Map param in (List<Map>)task.p){
		...
		switch((String)param.vt){
			...
			default:
				...
				String tt1=((String)param.vt).replace('[]',sBLK)
				def t0=v.v
...
// If the VirtualCommand exists and has o:true use that virtual command otherwise try the physical command
	...
	Long delay=lZ
	if((Boolean)rtD.eric) myDetail rtD,mySt+"$params"
...
private void executePhysicalCommand(Map rtD,device,String command,params=[],Long idel=lZ,String scheduleDevice=sNULL,Boolean dco=false){
	Long delay=idel
	if(delay!=lZ && scheduleDevice!=sNULL){
		...
		Integer ps= (String)statement.tcp in [sB,sP] ? 1:0
		...
		try{
			...
			def a
			while (psz>0 && nparams[psz-1]==null){ a=nparams.pop(); psz=nparams.size() }
			Map msg=null
			Boolean doL=(Integer)rtD.logging>2
			if(doL) msg=timer sBLK,rtD
			Boolean skip=false
			if(!rtD.piston.o?.dco && !dco && !(command in [sSCLRTEMP,sSCLR,sSHUE,sSSATUR])){
		...
		Long t0=rtD.piston.o?.ced ? ((Integer)rtD.piston.o.ced).toLong():lZ
		if(t0!=lZ){
			Long t1=(Long)getPistonLimits.deviceMaxDelay
			t0=t0>t1 ? t1:t0
...
private void scheduleTimer(Map rtD,Map timer,Long lastRun=lZ){
	...
	Long delta=lZ
	switch(intervalUnit){
		case sMS: level=1; delta=1L; break
		case sS: level=2; delta=lTHOUS; break
		case sM: level=3; delta=dMSMINT.toLong(); break
	...
	if(delta==lZ){
		...
		//resulting is sDTIME
		if(lastRun==lZ) //first run, just adjust the time so we're in the future
	...
	while(cycles!=0){
		if(delta!=lZ){
			if(nextSchedule<(rightNow-delta)){
				//we're behind, catch up to where the next future occurrence
			...
			//advance ahead of rightNow if we're in the past
			...
			switch(intervalUnit){
				...
				case 'y':
					...
					if(odw==sD){
						if(odm>0) day=(odm<=lastDayOfMonth)? odm:0
						else{
...
private void scheduleTimeCondition(Map rtD,Map condition){
	...
	//if already scheduled once during this run, don't do it again
	if(((List<Map>)rtD.schedules).find{ Map it -> (Integer)it.s==conditionNum && (Integer)it.i==0 })return
	...
	Integer pCnt=comparison.p!=null ? (Integer)comparison.p:0
	if(!pCnt)return
	...
	Map tv2=condition.ro2!=null && (String)condition.ro2.t!=sC && pCnt>1 ? (Map)evaluateOperand(rtD,null,(Map)condition.to2):null
	Long v2=trigger ? v1 : (pCnt>1 ? ((Long)evaluateExpression(rtD,(Map)evaluateOperand(rtD,null,(Map)condition.ro2,null,false,true),sDTIME).v + (tv2!=null ? (Long)evaluateExpression(rtD,rtnMap1(sDURATION,tv2.v,(String)tv2.vt)).v : lZ)) : (String)condition.lo.v==sTIME ? getMidnightTime():v1 )
...
private static Long checkTimeRestrictions(Map rtD,Map operand,Long time,Integer level,Integer interval){
	...
	// on minute of hour
	List<Integer> om=level<=2 && operand.om instanceof List && ((List)operand.om).size()>0 ? (List<Integer>)operand.om:null
	// on hours
	List<Integer> oh=level<=3 && operand.oh instanceof List && ((List)operand.oh).size()>0 ? (List<Integer>)operand.oh:null
	// on day(s) of week
	List<Integer> odw=level<=5 && operand.odw instanceof List && ((List)operand.odw).size()>0 ? (List<Integer>)operand.odw:null
	// on day(s) of month
	List<Integer> odm=level<=6 && operand.odm instanceof List && ((List)operand.odm).size()>0 ? (List<Integer>)operand.odm:null
	// on weeks of month
	List<Integer> owm=level<=6 && odm==null && operand.owm instanceof List && ((List)operand.owm).size()>0 ? (List<Integer>)operand.owm:null
	// on month of year
	List<Integer> omy=level<=7 && operand.omy instanceof List && ((List)operand.omy).size()>0 ? (List<Integer>)operand.omy:null
	...
	if(owm!=null && !((owm.indexOf(getWeekOfMonth(date))>=0) || (owm.indexOf(getWeekOfMonth(date,true))>=0))){
		switch(level){
			case 2: //by second
	...
	if(odm!=null && odm.indexOf(dyDate)<0){
...
private static Integer getWeekOfMonth(Date date,Boolean backwards=false){
	...
	}else return 1+Math.floor((day-1)/7) //1 based
}
...
private Long do_setLevel(Map rtD,device,List params,String attr,Integer val=null){
	...
	String mat=psz>1 ? (String)params[1]:sNULL
...
private Long cmd_setHue(Map rtD,device,List params){
	Integer hue=(Integer)cast(rtD,Math.round((Integer)params[0]/d3d6),sINT)
...
private Map getColor(Map rtD,String colorValue){
	...
	if(color!=null){
		color=[
			...
			hue:Math.round((Integer)color.h/d3d6),
		...
		if(color!=null){
			color=[
				...
				hue:Math.round((Integer)color.hue/d3d6),
...
private Long cmd_setAdjustedColor(Map rtD,device,List params){
	...
	Integer psz=params.size()
	String mat=psz>2 ? (String)params[2]:sNULL
	if(mat!=sNULL && (String)getDeviceAttributeValue(rtD,device,sSWITCH)!=mat) return lZ
...
private Long cmd_setAdjustedHSLColor(Map rtD,device,List params){
	Integer psz=params.size()
	String mat=psz>4 ? (String)params[4]:sNULL
	if(mat!=sNULL && (String)getDeviceAttributeValue(rtD,device,sSWITCH)!=mat) return lZ

	Long duration=(Long)cast(rtD,params[3],sLONG)
	Integer hue=(Integer)cast(rtD,Math.round((Integer)params[0]/d3d6),sINT,sLONG)
	...
	Long delay=psz>5 ? (Long)params[5]:lZ
	executePhysicalCommand(rtD,device,'setAdjustedColor',[color,duration],delay)
	return lZ
}

private Long cmd_setLoopDuration(Map rtD,device,List params){
	Integer duration=Math.round((Long)cast(rtD,params[0],sLONG)/dTHOUS).toInteger()
...
private Long vcmd_setLocationMode(Map rtD,device,List params){
	...
	def mode=((List)location.getModes())?.find{ (hashId((Long)it.id)==modeIdOrName)|| ((String)it.name==modeIdOrName)}
...
private Long vcmd_setAlarmSystemStatus(Map rtD,device,List params){
	...
	if(status && status.size()!=0) sendLocationEvent((sNM):'hsmSetArm',(sVAL):status[0].id)
	else error "Error setting HSM status. Status '$statusIdOrName' does not exist.",rtD
	return lZ
}
...
private Long vcmd_waitRandom(Map rtD,device,List params){
	...
	return min+Math.round(dONE*(max-min)*Math.random())
}
...
private Long vcmd_toggleRandom(Map rtD,device,List params){
	...
	executePhysicalCommand(rtD,device,(Integer)Math.round(100.0D*Math.random()).toInteger()<=probability ? sON : sOFF)
	return lZ
}
...
private Long do_fadeLevel(Map rtD,device,List params,String attr,String attr1,Integer val=null,Integer val1=null,Boolean big=false){
	...
	String mat=params.size()>3 ? (String)params[3]:sNULL
	if(mat!=sNULL && (String)getDeviceAttributeValue(rtD,device,sSWITCH)!=mat) return lZ
...
private Long vcmd_fadeHue(Map rtD,device,List params){
	Integer startLevel=(params[0]!=null)? (Integer)cast(rtD,Math.round((Integer)params[0]/d3d6),sINT,sLONG):(Integer)cast(rtD,getDeviceAttributeValue(rtD,device,sHUE),sINT)
	Integer endLevel=(Integer)cast(rtD,Math.round((Integer)params[1]/d3d6),sINT,sLONG)
...
private Long vcmd_flashLevel(Map rtD,device,List params){
	...
	String mat=params.size()>5 ? (String)params[5]:sNULL
	String currentState=(String)getDeviceAttributeValue(rtD,device,sSWITCH)
	if(mat!=sNULL && currentState!=mat) return lZ
...
private Long vcmd_sendNotification(Map rtD,device,List params){
	String message="Hubitat does not support sendNotification "+(String)params[0]
...
private Long vcmd_setHSLColor(Map rtD,device,List params){
	...
	String mat=params.size()>3 ? (String)params[3]:sNULL
	if(mat!=sNULL && (String)getDeviceAttributeValue(rtD,device,sSWITCH)!=mat)return lZ
	Long delay=params.size()>4 ? (Long)params[4]:lZ
	executePhysicalCommand(rtD,device,sSCLR,color,delay)
	return lZ
}
...
private Long do_lifx(Map rtD,String cmd,String path,Map body,duration,String c){
	...
	try{
		...
		Long ldur=duration ? Math.round( duration * dTHOUS) : lZ
...
private Long vcmd_lifxScene(Map rtD,device,List params){
	...
	Long duration=params.size()>1 ? Math.round( ((Long)cast(rtD,params[1],sLONG) / dTHOUS)) : lZ
...
private static String ffwding(Map rtD){
	if((Integer)rtD.ffTo==0)return sBLK
		...
		if((Integer)rtD.ffTo==0 || (Integer)rtD.ffTo==myC){
			...
			if(ladderIndex>=steps) value=false
				...
				if(ladderUpdated && duration!=lZ && (ladderUpdated+duration<(Long)now())){
					...
					if(!value) if(lg>2)debug "Conditional ladder step failed due to a timeout",rtD
				}else{
	...
	switch((String)operand.t){
		...
		case sV: //virtual devices
			...
			switch(oV){
				...
				case 'alarmSystemAlert':
					...
					mv=rtnMap(sSTR,(rEN=='hsmAlert' ? valStr:sNULL))
					break
				case "hsmSetArm":
				case 'alarmSystemEvent':
					mv=rtnMap(sSTR,(rEN=='hsmSetArm' ? evntVal:sNULL))
					break
		...
		case sX: //variable
			if(ovt==sDEV && operand.x instanceof List){
				...
				if(hasI)movt=ovt ? [(sVT):ovt.replace('[]',sBLK)] : [:]
	...
	if((Integer)rtD.ffTo!=0 || comparison!=null){
		...
		if((Integer)rtD.ffTo==0 || ((Integer)rtD.ffTo==-9 /*initial run*/)){
			...
			if((Boolean)rtD.eric && trigger && (Integer)rtD.ffTo==0 && (Integer)rtD.statementLevel!=1)
				myDetail rtD,"trigger comparison ${co} at level > 1  level: ${rtD.statementLevel}"
			...
			if(t_and_compt && (Integer)rtD.ffTo==0){
				...
				if(eric())log.debug "stays check ${co} isStays: $isStays result: $result"
				if(to!=null){
...
private void updateCache(Map rtD,Map value){
	...
	if(oldValue==null || ((String)oldValue.t!=(String)value.v.t) || (oldValue.v!=value.v.v)){
		((Map)rtD.newCache)[(String)value.i]=(Map)value.v+[(sS):(Long)now()]
	}
}
...
//if((Boolean)rtD.eric) myDetail rtD,"$r2res  ${myObj(value?.v?.v)}    ${myObj(rvalue?.v?.v)}  $fn $value   $rvalue    $r2value    $tvalue   $tvalue2",-1
			...
			if(res && (String)lo.operand.t==sV && (String)lo.operand.v in LT1){
				Boolean pass=(checkTimeRestrictions(rtD,(Map)lo.operand,(Long)now(),5,1)==lZ)
...
private static Map valueCacheChanged(Map rtD,Map comparisonValue){
	...
	private static Boolean match(String str,String pattern){
		...
		if(sz>2 && pattern.startsWith(sDIV) && pattern.endsWith(sDIV)){
	...
	if(node instanceof List){
		for(item in (List<Map>)node)
			if(!item.di){
				...
				if(data!=null && (String)item.t==sEVERY) data.timer=true
	...
	try{
		...
		for(subscription in subscriptions){
			...
			if(!rtD.piston.o?.des && (String)subscription.value.t && !!subscription.value.c && altSub!=never && ((String)subscription.value.t==sTRIG || altSub==always || !hasTriggers)){
				...
				for(condition in (List<Map>)subscription.value.c){
					if(condition){ condition.s=false } // modifies the code
				}
				if(devices[devStr]) devices[devStr].c=(Integer)devices[devStr].c-1
			}
		}
		//not using fake subscriptions for utilized devices - piston has device in settings
		for(d in devices.findAll{ ((Integer)it.value.c<=0 || rtD.piston.o?.des) && (String)it.key!=(String)rtD.locationId }){
			...
			if(device!=null && !isDeviceLocation(device)){
				String didS=device.id.toString()
				if(lg>1 && doit)trace "Piston utilizes $device...",rtD
				ss.controls+=1
				if(!dds[didS]){
					ss.devices+=1
					dds[didS]=1
		...
		if(doit){
			...
			Map event=[(sDATE):new Date(),(sDEV):location,(sNM):sTIME,(sVAL):(Long)now(),schedule:[(sT):lZ,(sS):0,(sI):-9]]
...
private getDeviceAttributeValue(Map rtD,device,String attributeName){
	...
	if(rtDEvN==attributeName && rtDEdID) return rtD.event.value
	else{
		def result
...
private Map getDeviceAttribute(Map rtD,String deviceId,String attributeName,subDeviceIndex=null,Boolean trigger=false){
	...
	if(device!=null){
		...
		if(attribute==null) attribute=[(sT):sSTR, /* m:false */ ]
...
//	if(attributeName)log.warn "attributeName: $attributeName  t0:  ($tt2) $t0    tt1: $tt1"
		...
		if(attributeName==sHUE) value=cast(rtD,(Double)cast(rtD,value,sDEC)*d3d6,(String)attribute.t)
				...
				if(feature=='NFL' && partIndex==1 && !!args && !!args.games){
					...
					if(start!=null){
						...
						if((Integer)parts[0].size()>3){
							def games=((List<Map>)args.games).findAll{ Map it -> (it.year*372+(it.month-1)*31+(it.day-1)>=start) && (it.year*372+(it.month-1)*31+(it.day-1)<=end)}
							args=games
							overrideArgs=true
						}else{
							def game=((List<Map>)args.games).find{ Map it -> (it.year*372+(it.month-1)*31+(it.day-1)>=start) && (it.year*372+(it.month-1)*31+(it.day-1)<=end)}
							args=game
							continue
						}
					}
...
//	if(eric())log.debug "getVariable ${name} ${tn} ${var}"
	...
	if(tn.startsWith(sAT)){
		...
		if(tn.startsWith(sDLR)){
			Integer t0=tn.size()
			if(tn.startsWith(sDOLARGS+sDOT) && (t0>6)){ // '$args.'
				res=getArgument(rtD,tn.substring(6))
			}else if(tn.startsWith(sDOLARGS+sLB) && (t0>6)){ //'$args['
				res=getArgument(rtD,tn.substring(5))
			...
			}else if(tn.startsWith(sDOLJSON+sDOT) && (t0>6)){
				res=getJson(rtD,tn.substring(6))
			}else if(tn.startsWith(sDOLJSON+sLB) && (t0>6)){
				res=getJson(rtD,tn.substring(5))
			}else if(tn.startsWith('$incidents.') && (t0>11)){
				res=getIncidents(rtD,tn.substring(11))
			}else if(tn.startsWith('$incidents[') && (t0>11)){
				res=getIncidents(rtD,tn.substring(10))
			}else if(tn.startsWith('$nfl.') && (t0>5)){
				res=getNFL(rtD,tn.substring(5))
...
//			if(eric())log.debug "getVariable ${rtD.localVars}"
	...
	if(res!=null && ((String)res.t).endsWith(sRB)){
		res.t=((String)res.t).replace('[]',sBLK)
	...
	res=rtnMap(t,v)
	if((Boolean)rtD.eric) myDetail rtD,mySt+"result:$res",-1
	res
}
...
private Map setVariable(Map rtD,String name,value){
	...
	String tn=sanitizeVariableName(var.name)
	if(tn==sNULL)return rtnMap(sERROR,'Invalid empty variable name')
	if((Boolean)rtD.eric) myDetail rtD,"setVariable ${tn} value: ${value}"
				...
				if(wctyp){ // if we know current type
					...
					ta.each{
						...
						if(a){
							...
							if((Boolean)rtD.eric) myDetail rtD,"setVariable returning ${result} to webcore"
...
private Map evaluateExpression(Map rtD,Map express,String dataType=sNULL){
	...
	switch(exprType){
		...
		case sDURATION:
			...
			if(t0==null && exprV instanceof Long){ result=rtnMap(sLONG,(Long)exprV) }
		...
		case sEXPR:
			...
			for(Map item in (List<Map>)expression.i){
				if((String)item.t==sOPER){
					...
					if(operand<0){
						if(ito in L1opt){
							a=items.push(rtnMap(sINT,0)+mito)
						} else switch(ito){
							case sCOLON:
								if(lastOperand>=0){
									//groovy-style support for(object ?: value)
									a=items.push(items[lastOperand]+mito)
								}else a=items.push(rtnMap(sINT,0)+mito)
								break
							case sMULP:
							case sDIV:
								a=items.push(rtnMap(sINT,1)+mito)
								break
							case sLAND:
							case sLNAND:
			...
			while (itmSz>1){
				...
				if(itmSz==3 && (String)items[0].o==sQM && (String)items[1].o==sCOLON){
					...
					if((Boolean)evaluateExpression(rtD,(Map)items[0],sBOOLN).v) items=[items[1]]
					else items=[items[2]]
					items[0].o=sNULL
					break
				}
			...
			result=items[0] ? ((String)items[0].t==sDEV ? (Map)items[0] : evaluateExpression(rtD,(Map)items[0])) : rtnMap(sDYN,null)
			break
	} //end switch
	...
	if(dataType && dataType!=sDEV && (String)result.t==sDEV){
		...
		switch((Integer)atL.size()){
			case 0: result=rtnMap(sERROR,'Empty device list'); break
...
private Map func_dewpoint(Map rtD,List<Map> params){
	...
	if((rh>dZERO) && (rh<dONE)) rh=rh*100.0D
...
private Map func_integer(Map rtD,List<Map> params){
	...
	rtnMap(sINT,(Integer)evaluateExpression(rtD,params[0],sINT).v)
}
...
private Map func_decimal(Map rtD,List<Map> params){
	...
	rtnMap(sDEC,(Double)evaluateExpression(rtD,params[0],sDEC).v)
}
...
private Map func_boolean(Map rtD,List<Map> params){
	...
	rtnMap(sBOOLN,(Boolean)evaluateExpression(rtD,params[0],sBOOLN).v)
}
...
private Map func_sqr(Map rtD,List<Map> params){
	...
	rtnMap(sDEC,(Double)evaluateExpression(rtD,params[0],sDEC).v**2)
}
...
private Map func_sqrt(Map rtD,List<Map> params){
	...
	rtnMap(sDEC,Math.sqrt((Double)evaluateExpression(rtD,params[0],sDEC).v))
}
...
private Map func_power(Map rtD,List<Map> params){
	...
	rtnMap(sDEC,(Double)evaluateExpression(rtD,params[0],sDEC).v ** (Double)evaluateExpression(rtD,params[1],sDEC).v)
}
...
private Map func_round(Map rtD,List<Map> params){
	...
	rtnMap(sDEC,Math.round((Double)evaluateExpression(rtD,params[0],sDEC).v * (10 ** precision))/(10 ** precision))
}
...
private Map func_floor(Map rtD,List<Map> params){
	...
	rtnMap(sINT,(Integer)cast(rtD,Math.floor((Double)evaluateExpression(rtD,params[0],sDEC).v),sINT))
}
...
private Map func_ceiling(Map rtD,List<Map> params){
	...
	rtnMap(sINT,(Integer)cast(rtD,Math.ceil((Double)evaluateExpression(rtD,params[0],sDEC).v),sINT))
}
...
private Map func_sprintf(Map rtD,List<Map> params){
	...
	try{
		...
		for(Integer x=1; x<sz; x++) a=args.push(evaluateExpression(rtD,params[x]).v)
...
private Map func_left(Map rtD,List<Map> params){
	...
	rtnMap(sSTR,value.substring(0,count))
}
...
private Map func_right(Map rtD,List<Map> params){
	...
	Integer sz=value.size()
	if(count>sz)count=sz
	rtnMap(sSTR,value.substring(sz-count,sz))
}
...
private Map func_substring(Map rtD,List<Map> params){
	...
	if(start<t0 && start>-t0){
		if(count!=null){
			...
			}else if(count>-start)count=-start
		}
		start=start>=0 ? start : t0+start
		if(count>t0-start)count=t0-start
...
private Map func_replace(Map rtD,List<Map> params){
	...
	for(Integer i=0; i<cnt; i++){
		...
		sz=search.size()
		if((sz>2)&& search.startsWith(sDIV)&& search.endsWith(sDIV)){
...
private Map func_median(Map rtD,List<Map> params){
	...
	if(data) return data[(Integer)Math.floor(data.size()/2)]
	rtnMap(sDYN,sBLK)
}
...
private Map func_least(Map rtD,List<Map> params){
	...
	for(Map param in params){
		...
		data[value.v]=rtnMap((String)value.t,value.v)+[(sC):(data[value.v]?.c ?: 0)+1]
	}
...
private Map func_variance(Map rtD,List<Map> params){
	...
	Double avg=sum/sz
	sum=dZERO
	for(Integer i=0; i<sz; i++) sum+=((Double)values[i]-avg)**2
	rtnMap(sDEC,sum/sz)
}
...
private Map func_stdev(Map rtD,List<Map> params){
	if(!checkParams(rtD,params,2)) return rtnErr('stdev'+sVALUEN)
...
private Map func_max(Map rtD,List<Map> params){
	...
	List<Map> data=params.collect{ Map it -> evaluateExpression(rtD,(Map)it,sDYN)}.sort{ Map it -> it.v }
	if(data) return data[data.size()-1]
	rtnMap(sDYN,sBLK)
}
...
private Map func_count(Map rtD,List<Map> params){
	...
	}else for(Map param in params) count+=(Boolean)evaluateExpression(rtD,param,sBOOLN).v ? 1:0
	rtnMap(sINT,count)
}
...
private Map func_datetime(Map rtD,List<Map> params){
	...
	Long value=sz>0 ? (Long)evaluateExpression(rtD,params[0],sDTIME).v:(Long)now()
	rtnMap(sDTIME,value)
}
...
private Map func_date(Map rtD,List<Map> params){
	...
	Long value=sz>0 ? (Long)evaluateExpression(rtD,params[0],sDATE).v:(Long)cast(rtD,(Long)now(),sDATE,sDTIME)
	rtnMap(sDATE,value)
}
...
private Map func_time(Map rtD,List<Map> params){
	...
	Long value=sz>0 ? (Long)evaluateExpression(rtD,params[0],sTIME).v:(Long)cast(rtD,(Long)now(),sTIME,sDTIME)
	rtnMap(sTIME,value)
}
...
private Map addtimeHelper(Map rtD,List<Map> params,Long mulp,String msg){
	...
	Long value=sz==2 ? (Long)evaluateExpression(rtD,params[0],sDTIME).v:(Long)now()
...
private Map func_formatduration(Map rtD,List<Map> params){
	...
	switch(granularity){
		...
		case sM: parts=3; partName='minute'; break
		case sMS: parts=5; partName='millisecond'; break
	...
	if(friendly){
		...
		switch(sz){
			...
			default:
				result=sBLK
				for(Integer i=0; i<sz; i++){
					result+=(i ? (sz>2 ? sCOMMA:sSPC):sBLK)+(i==sz-1 ? sAND+sSPC:sBLK)+p[i]
				}
...
private Map func_distance(Map rtD,List<Map> params){
	...
	if(idx<4 || idx>5)return rtnMap(sERROR,'Invalid parameter combination. Expecting either two devices,a device and two decimals,or four decimals,followed by an optional unit.')
	...
	switch(unit!=null ? unit:sM){
		...
		case 'kilometer':
		case 'kilometers':
			return rtnMap(sDEC,dist/dTHOUS)
		case 'mi':
		case 'mile':
		case 'miles':
...
private cast(Map rtD,ival,String dataT,String isrcDT=sNULL){
	...
	switch(dataType){
		...
		case sTIME:
			...
			Date t1=new Date(d)
			d=Math.round((t1.hours*dSECHR+(Integer)t1.minutes*dSIXTY+t1.seconds)*dTHOUS)
			return d
		case sDATE:
		case sDTIME:
			Long d
...
private Date utcToLocalDate(dateOrTimeOrString=null){ // this is really cast to Date
	...
	if(ldate instanceof Long){
		//HE is set to local timezone of hub by default, so we don't have to try to set timezone
			...
			case sTIME:
				...
				if(aaa!=null){
					if(aaa<lMSDAY && aaa>=lZ){
						...
						Long t2=Math.round((t1.hours*3600+t1.minutes*60+t1.seconds)*dTHOUS)
						myv=t2
		...
		switch(typ){
			...
			case sDTIME: // global times: everything is datetime -> these come in string and needs to be a long of appropriate type
				...
				if(mtyp==sTIME){
					...
					Long m2=Math.round((m1.hours*3600+m1.minutes*60+m1.seconds)*dTHOUS)
